...
/***********æŒ‡ä»¤è¯´æ˜Ž***************
*AT+
*NRB:æ¨¡å—é‡å¯                                  CMEE=1:æŠ¥å‘Šç§»åŠ¨ç»ˆç«¯é”™è¯¯            CFUN?:è®¾ç½®ç»ˆç«¯åŠŸèƒ½ 
*NBAND?:è®¾ç½®é¢‘æ®µ                               NCCID?:å¼€ç‰‡è¯†åˆ«                CGSN=1:æŸ¥è¯¢æ¨¡å—åºåˆ—å·
*CIMI:æŸ¥è¯¢å›½é™…ç§»åŠ¨è®¾å¤‡èº«ä»½ç                           CSQ:èŽ·å–ä¿¡å·å¼ºåº¦                 CEREG?:æŸ¥è¯¢ç½‘ç»œæ³¨å†ŒçŠ¶æ€ 
*CGPADDR:æ˜¾ç¤ºPDPåœ°å€                           CCLK?:è¿”å›žå½“å‰æ—¥æœŸå’Œæ—¶é—´            CSCON?:æŸ¥è¯¢ä¿¡å·è¿žæŽ¥çŠ¶æ€
*CGATT?:PSè¿žæŽ¥æˆ–åˆ†ç¦»                            NUESTATS:èŽ·å–çš„æ“ä½œç»Ÿè®¡           CGMR:æŸ¥è¯¢å›ºä»¶ç‰ˆæœ¬
*NSOCR=DGRAM,17,8888,1:åˆ›å»ºSocket,17-udp     NSOST:å‘é€æ•°æ®                 NSORF=0,256:æŽ¥æ”¶å‘½ä»¤ 0-NSOCRè¿”å›žå€¼ 256-max recive data len
*NSOCL=0:å…³é—­Socket                          NSOCR=DGRAM,17,8990,1/AT+NSOCR=DGRAM,17,8992,1-åˆ›å»ºä¸åŒç«¯å£çš„socket
*/ 
const sNB_CMD MyNBIOT_CMD_data[30] = 
{
	// 	Ö¸ÁîÄÚÈÝ									×î´ó³¢ÊÔ´ÎÊý		³¬Ê±Ê±¼äms		  		ÕýÈ··µ»ØÖµ			³õÊ¼×´Ì¬		Ö´ÐÐÖ¸Áî¼ä¸ôms
	{"",												3,					1000,				"",				NO_BACK,			100},
	{"AT\r\n",											5,					30,					"OK",			NO_BACK,			0},
	{"AT+NRB\r\n",										3,					7000,				"OK",			NO_BACK,			0},
	{"AT+CMEE=1\r\n",									3,					100,				"OK",			NO_BACK,			0},
 ...
 *@date:	2017-12-18
 *@brief:	
 *@return:	³É¹¦»òÕß´íÎó£¬Èç¹û´íÎó£¬´íÎóÀàÐÍÔÚpMyCMD->NB_Return
 */
...
void checkNBNetwork(pNBiotAT pMyNBAT)
{
	...
	if(DoNBiotATCmd(pMyNBAT, AT_CEREG, JUDGE_AND, "CEREG:0,1", pMyNBAT->ATCMDData) == SUCCESS)	//×¢²áÍøÂç³É¹¦
	{
...
//	MyNBIOT_CMD_data[AT_CFUN].pTrueBack = "+CFUN:1";
	...
	{
		pMyNB->network_status &= (~0x02);	//²»ÄÜ´ò¿ªÉäÆµ£¬¿ÉÄÜSIM¿¨Ã»²åºÃ
		return pMyNB->network_status;
	}
	
	// ÉèÖÃAPN
	
	if(getCSQ(pMyNB) == SUCCESS)			// »ñÈ¡ÐÅºÅÇ¿¶È
	{
		pMyNB->network_status |= 0x04;
	}
	else
	{
		pMyNB->network_status &= (~0x04);	//²»ÄÜ´ò¿ªÉäÆµ£¬¿ÉÄÜSIM¿¨Ã»²åºÃ
		return pMyNB->network_status;
	}
...
void check_socket_status(pNB pMyNB)
{
	...
	pMyNB->default_server.status |= 0x80;		//´ò¿ªÄ¬ÈÏ·þÎñÆ÷socket
#ifdef DEFAULT_SERVER_ADDR	//ºê¶¨ÒåÔÚMyNBIOT.h ÎÄ¼þÖÐ µ÷ÊÔÓÃÄ¬ÈÏ·þÎñÆ÷
	uint16_t tmp[5];
...
ErrorStatus checkNBRecvBuff(pNB pMyNB)
{
	...
	while(try_cnt < 1 && (pMyNB->nb_recv_data.receive_status & 0x40) != 0x40)
	{
		...
		{	
			...
			{
				pMyNB->nb_recv_data.receive_status |= 0x40;	//ÊÕµ½Êý¾Ý
